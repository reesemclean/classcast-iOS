...
@interface RDMEditGroupViewController_iPad ()

@property (weak, nonatomic) IBOutlet UILabel *instuctionsLabel;
...
- (IBAction)registrationCodeRefreshButtonPushed:(id)sender;

@property (nonatomic, strong) RDMTeacherAccountController *accountController;
@end

@implementation RDMEditGroupViewController_iPad
...
- (void)viewDidLoad
{
    ...
    if (RDM_IS_IOS7_BASED_ON_VIEW(self.view)) {
        ...
        self.instuctionsLabel.font = [UIFont fontWithName:@"HelveticaNeue-Light" size:14.0];
        self.registrationCodeLabel.font = [UIFont fontWithName:@"HelveticaNeue-Light" size:24.0];
    }
    
    self.temporaryContext = [self.dataController createTemporaryContext];
    
    NSError *error = nil;
    self.groupInTemporaryContext = (RDMGroup*)[self.temporaryContext existingObjectWithID:self.groupToEdit.objectID
                                                                                  error:&error];
    if (error) {
        NSLog(@"Could Not Find Group: %@", error);
    }
    
    self.registrationCodeLabel.text = self.groupInTemporaryContext.registrationToken;
    self.nameTextField.text = self.groupInTemporaryContext.name;
    ...
    self.syncCompleteObserver = [[NSNotificationCenter defaultCenter] addObserverForName:RDMSyncEngineDidFinishNotification
                                                                              ...
                                                                              usingBlock:^(NSNotification * note) {
                                                                                  ...
                                                                                  if ([self.groupToEdit.hasBeenDeleted boolValue] || self.groupToEdit.isDeleted || !self.groupToEdit.managedObjectContext) {
                                                                                      [self dismissViewControllerAnimated:YES completion:nil];
                                                                                  }
                                                                                  
                                                                            }];
    self.selectedDevices = [self.groupInTemporaryContext.devices mutableCopy];
}
...
- (IBAction)saveButtonPushed:(id)sender {
    ...
    NSMutableSet *unchangedDevices = [self.selectedDevices mutableCopy];
    [unchangedDevices intersectSet:self.groupInTemporaryContext.devices];
    
    NSMutableSet *removedDevices = [self.groupInTemporaryContext.devices mutableCopy];
    [removedDevices minusSet:unchangedDevices];
    
    NSMutableSet *addedDevices = [self.selectedDevices mutableCopy];
    [addedDevices minusSet:unchangedDevices];
    
    for (RDMDevice *device in removedDevices) {
        ...
        groupPlacement.placementType = @(RDMGroupPlacementTypeRemoved);
        groupPlacement.user = device.user;
        
    }
    
    for (RDMDevice *device in addedDevices) {
    ...
    [self.groupInTemporaryContext setDevices:self.selectedDevices];
...
-(IBAction) deleteButtonPushed:(id) sender {
    ...
    UIActionSheet *actionSheet = [[UIActionSheet alloc] initWithTitle:@"Confirm delete — the devices will not be unregistered."
                                                             delegate:self
    ...
    [actionSheet showFromRect:button.superview.frame
                       inView:self.view
                     animated:YES];
...
-(void) actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex {
    ...
    if (actionSheet.destructiveButtonIndex == buttonIndex) {
        ...
        self.groupInTemporaryContext.hasBeenDeleted = @YES;
        self.groupInTemporaryContext.syncStatus = @1;
...
- (IBAction)nameTextFieldDidChange:(id)sender {
    ...
    self.groupInTemporaryContext.name = self.nameTextField.text;
...
- (IBAction)registrationCodeRefreshButtonPushed:(id)sender {
    ...
    [SVProgressHUD showWithStatus:@"Changing Registration Code"];
    
    [self.accountController sendGroupRegistrationTokenChangeRequestForGroup:self.groupToEdit
                                                             WithCompletion:^(RDMGroup *group, NSError *error) {
                                                                 
                                                                 if (group) {
                                                                     
                                                                     [SVProgressHUD showSuccessWithStatus:@"All Done!"];
                                                                     
                                                                     self.groupInTemporaryContext.registrationToken = group.registrationToken;
                                                                     self.registrationCodeLabel.text = group.registrationToken;
                                                                 } else {
...
-(NSFetchedResultsController*) devicesFetchedResultsController {
    ...
    NSPredicate *userPredicate = [NSPredicate predicateWithFormat:@"user == %@", self.groupInTemporaryContext.user];
